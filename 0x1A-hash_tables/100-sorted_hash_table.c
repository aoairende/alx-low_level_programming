...
shash_table_t *shash_table_create(unsigned long int size)
{
	...
	ht->size = size;
	/* Allocate memory for the array of sorted hash node pointers. */
 ...
 * @key: A pointer to the key to be added.
...
int shash_table_set(shash_table_t *ht, const char *key, const char *value)
{
	...
	/* Check for NULL pointers or an empty key. */
	...
	/* Allocate memory for a new sorted hash node. */
...
void shash_table_print_rev(const shash_table_t *ht)
{
	...
	node = ht->stail;
	printf("{");
	/* Iterate through the sorted linked list in reverse order. */
	while (node != NULL)
	{
		printf("'%s': '%s'", node->key, node->value);
		node = node->sprev;
